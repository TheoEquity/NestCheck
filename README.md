...
个人电脑长期使用推荐走生产模式：由后端统一托管已经构建好的 Web 静态资源，日常访问只打开一个本地服务端口。开发模式的 `npm run dev` 仅用于前端开发调试。

#### 方案 A：Windows + Docker Desktop

适合希望运行环境更干净、升级更稳定的个人电脑部署。
...
#### 方案 B：Windows + Python 生产运行

适合希望直接用源码目录运行的个人电脑部署。
...
4. 让 NestCheck 监听可被 Tailscale 访问的地址。

Docker Compose 方式可在 `.env` 中设置：
...
Windows 防火墙首次弹窗时选择允许“专用网络”。也可以手动放行端口：

```powershell
New-NetFirewallRule -DisplayName "NestCheck Tailscale 8000" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 8000 -Profile Private
```
