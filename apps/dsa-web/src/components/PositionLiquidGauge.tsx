...
interface PositionLiquidGaugeProps {
  ...
  /** Active allocation plan equity ratio (0.0–1.0), rendered as red dotted line */
...
export const PositionLiquidGauge: React.FC<PositionLiquidGaugeProps> = ({ data, currentRatio, plannedRatio }) => {
  ...
  const plannedPercent = plannedRatio != null ? Math.round(plannedRatio * 100) : null;
    ...
    <>
      ...
      <div className="flex items-center justify-center" style={{ height: 220 }}>
        <div className="relative h-44 w-72 shrink-0">
          <div className="absolute left-1/2 top-0 h-44 w-44 -translate-x-1/2 rounded-full border-2 border-border overflow-hidden shadow-xl"
          ...
          {currentPercent != null && (
            <div
              className="absolute left-14 right-14 z-20 border-t-[3px] border-dashed border-white/75"
          ...
          {currentPercent != null && (
            <div
              className="absolute right-0 z-20 text-[10px] font-bold text-secondary-text bg-card/90 px-1.5 py-0.5 rounded border border-border shadow-sm"
          ...
          {plannedPercent != null && (
            <div
              className="absolute left-14 right-14 z-30 border-t-[4px] border-dashed border-red-500"
              style={{ bottom: `${plannedPercent}%` }}
              title={`计划水位 ${plannedPercent}%`}
            />
          )}
          {/* 计划水位标签 */}
          {plannedPercent != null && (
            <div
              className="absolute left-0 z-30 text-[10px] font-bold text-red-600 bg-card/90 px-1.5 py-0.5 rounded border border-red-500/40 shadow-sm"
              style={{ bottom: `${Math.min(plannedPercent + 1, 95)}%` }}
            >
              计划水位 {plannedPercent}%
            </div>
          )}
      ...
      <div className="mt-1 border-t border-border/30 px-1 pt-1.5 text-[11px] leading-snug text-secondary-text">
        <div>
          ...
          {gaugeData.isValid ? (
            ...
            {plannedPercent != null ? <>，计划水位 <span className="text-foreground font-medium">{plannedPercent}%</span></> : null}。</>
          ) : (
            <>数据待完善...</>
          )}
        </div>
