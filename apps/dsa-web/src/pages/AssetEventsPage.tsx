...
type LedgerEventType = 'all' | 'trade' | 'cash' | 'corporate_action';
type SubmitTarget = 'trade' | 'cash' | 'corporate' | 'split';
type ActiveEventForm = SubmitTarget | null;
...
const EVENT_FORM_OPTIONS: Array<{ value: SubmitTarget; label: string; description: string }> = [
  { value: 'trade', label: '交易事件', description: '买入、卖出、持仓调整' },
  { value: 'split', label: '送股除权', description: '每10股送股除权' },
  { value: 'cash', label: '资金流水', description: '账户现金流入流出' },
  { value: 'corporate', label: '现金分红', description: '持仓标的现金分红' },
];
...
const AssetEventsPage: React.FC = () => {
  ...
  const [activeEventForm, setActiveEventForm] = useState<ActiveEventForm>(null);
  ...
  const [fundResetConfirmOpen, setFundResetConfirmOpen] = useState(false);
  const [fundResetConfirmMsg, setFundResetConfirmMsg] = useState('');
  const [fundResetLoading, setFundResetLoading] = useState(false);
  ...
  const loadFundStatus = useCallback(async () => {
    try {
      const status = await portfolioApi.getFundStatus();
      setFundStatus(status);
    } catch {
      // fund status is optional, silently ignore
    }
  }, []);
    ...
    const loadCategories = async () => {
      try {
        ...
        if (definitions.length > 0) {
          setTradeForm((prev) => {
            if (definitions.some((item) => item.code === prev.assetCategory)) return prev;
            const first = definitions[0];
            return {
              ...prev,
              assetCategory: first.code,
              assetSubcategory: '',
              assetRiskClass: (first.defaultRiskClass || prev.assetRiskClass) as AssetRiskClass,
            };
          });
        }
      } catch (err) {
  ...
  const stockSplitRatio = Number(splitForm.splitRatio || 0);

  const selectedAccountPositions = useMemo(
    () => positions.filter((item) => item.accountId === selectedAccountId && item.quantity > 0),
    [positions, selectedAccountId],
  );

  const selectedDividendPositions = useMemo(
    () => selectedAccountPositions.filter((item) => item.assetCategory !== 'cash'),
    [selectedAccountPositions],
  );

  const selectedStockPositions = useMemo(
    () => selectedAccountPositions.filter((item) => item.assetCategory === 'stock'),
    [selectedAccountPositions],
  );

  const selectedSellPositions = useMemo(
    () => selectedAccountPositions.filter((item) => {
      if (item.assetCategory !== tradeForm.assetCategory) return false;
      if (tradeForm.assetSubcategory && item.assetSubcategory !== tradeForm.assetSubcategory) return false;
      return true;
    }),
    [selectedAccountPositions, tradeForm.assetCategory, tradeForm.assetSubcategory],
  );

  const selectedCorporatePosition = useMemo(
    () => selectedDividendPositions.find((item) => item.symbol === corpForm.symbol) || null,
    [corpForm.symbol, selectedDividendPositions],
  );

  const selectedSplitPosition = useMemo(
    () => selectedStockPositions.find((item) => item.symbol === splitForm.symbol) || null,
    [splitForm.symbol, selectedStockPositions],
  );
  ...
  const handleSellPositionChange = (symbol: string) => {
    const position = selectedSellPositions.find((item) => item.symbol === symbol);
    updateTradeForm({ symbol, name: position?.name || '' });
  };
  ...
  const updateSplitForm = (patch: Partial<typeof splitForm>) => {
    setSplitForm((prev) => ({ ...prev, ...patch }));
  };
  ...
  const buildLedgerQuery = (eventType: LedgerEventType) => {
    const query: { accountId?: number; symbol?: string; dateFrom?: string; dateTo?: string; page: number; pageSize: number } = {
      page: 1,
      pageSize: 50,
    };
  ...
  const ledgerRows = useMemo<LedgerRow[]>(() => {
    ...
    recentCorporateActions.forEach((item) => {
      ...
      const isStockSplit = item.actionType === 'stock_split';
      rows.push({
        key: `corp-${item.id}`,
        date: item.effectiveDate,
        typeLabel: isStockSplit ? '送股除权' : '现金分红',
        accountId: item.accountId,
        accountName: accountNameById.get(item.accountId) || '--',
        symbol: item.symbol,
        name: positionNameByAccountSymbol.get(`${item.accountId}:${item.symbol}`) || '--',
        direction: isStockSplit ? '除权' : '现金分红',
        amount: isStockSplit ? `每10股送${formatNumber(item.splitRatio || 0)}股` : `${item.currency} ${formatNumber(dividendAmount)}`,
        price: '--',
        note: item.note || (isStockSplit ? '送股除权，数量增加，成本与当前价按比例下调' : `现金分红，本单盈利${formatNumber(dividendAmount)}元，不影响成本`),
      });
    });
  ...
  const refreshLedger = async () => {
    ...
    if (ledgerFilters.eventType === 'all' || ledgerFilters.eventType === 'corporate_action') {
      requests.push(portfolioApi.listCorporateActions(buildLedgerQuery('corporate_action')).then((data) => setRecentCorporateActions(data.items)));
    } else {
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await portfolioApi.createTrade({
        ...
        availableDate: tradeForm.availableDate || undefined,
        side: tradeForm.side,
      ...
      setTradeForm((prev) => ({ ...prev, symbol: '', name: '', availableDate: '', quantity: '', price: '', fee: '', tax: '', note: '' }));
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
      setSubmitting(null);
  ...
  const handleCashSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Promise.all([refreshRecentCashLedgers(), refreshPositions(), loadFundStatus()]);
  ...
  const handleSplitSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedAccount || !selectedSplitPosition) {
      setError({ title: '请选择股票标的', message: '请先选择当前账户下已有股票持仓标的。', rawMessage: 'stock position is required', category: 'missing_params' });
      return;
    }
    setSubmitting('split');
    setError(null);
    ...
    try {
      await portfolioApi.createCorporateAction({
        accountId: selectedAccount.id,
        symbol: selectedSplitPosition.symbol,
        assetCategory: selectedSplitPosition.assetCategory || undefined,
        assetSubcategory: selectedSplitPosition.assetSubcategory || undefined,
        effectiveDate: splitForm.effectiveDate,
        actionType: 'stock_split',
        market: selectedSplitPosition.market as AccountMarket,
        currency: selectedSplitPosition.currency,
        splitRatio: Number(splitForm.splitRatio),
        note: splitForm.note.trim() || undefined,
      });
      await Promise.all([refreshRecentCorporateActions(), refreshPositions(), loadFundStatus()]);
      setSuccessMessage('送股除权事件已写入，持仓数量、成本和当前价已按比例调整。');
      setSplitForm((prev) => ({ ...prev, splitRatio: '', note: '' }));
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
      setSubmitting(null);
  ...
  const handleFundResetConfirm = useCallback(async () => {
    setFundResetConfirmOpen(false);
    setFundResetLoading(true);
    try {
      await portfolioApi.resetFund();
      setSuccessMessage('净值已重置，单位净值恢复为1.0000元。');
      await loadFundStatus();
      await refreshPositions();
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
      setFundResetLoading(false);
    }
  }, [loadFundStatus, refreshPositions]);
    ...
    <AppPage className="max-w-[1600px] space-y-3">
      <PageHeader
        ...
        actions={(
          <div className="flex items-center gap-2">
            <Button type="button" variant="secondary" size="sm" disabled={fundResetLoading} onClick={() => {
              const totalEquity = fundStatus?.totalEquity ?? 0;
              const hasEstablished = (fundStatus?.latestShares ?? 0) > 0;

              const sharesStr = formatNumber(totalEquity);
              if (!hasEstablished) {
                setFundResetConfirmMsg(`建立内部基金，当前净值为1.0000元，资产份额为 ${sharesStr} (即总资产)`);
              } else {
                setFundResetConfirmMsg(`净值重置将单位净值恢复为1元，资产份额为 ${sharesStr} (当前总资产)`);
              }
              setFundResetConfirmOpen(true);
            }}>
              {fundResetLoading ? '重置中...' : '净值重置'}
            </Button>
      ...
      <Card className="!rounded-xl" padding="sm">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-secondary">Event Console</p>
            <h2 className="mt-1 text-base font-semibold text-foreground">事件操作</h2>
            <p className="mt-1 text-sm text-secondary">选择要录入的事件，表单会展开在交易台账上方。</p>
          </div>
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
            {EVENT_FORM_OPTIONS.map((option) => {
              const active = activeEventForm === option.value;
              return (
                <button
                  key={option.value}
                  type="button"
                  className={`rounded-lg border px-3 py-2 text-left transition-all ${active ? 'border-primary/70 bg-primary/10 text-foreground shadow-sm' : 'border-border/40 bg-background/40 text-secondary hover:border-primary/40 hover:text-foreground'}`}
                  onClick={() => setActiveEventForm(active ? null : option.value)}
                >
      ...
      <section className="space-y-2">
          ...
          <form className="flex h-full flex-col gap-2" onSubmit={handleTradeSubmit}>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="text-xs text-secondary">资产大类
                <select className={`${SELECT_CLASS} mt-1`} value={tradeForm.assetCategory} onChange={(e) => {
                  ...
                  updateTradeForm({
                    ...
                    ...(tradeForm.side === 'sell' ? { symbol: '', name: '' } : {}),
                  });
                }}>
              ...
              <label className="text-xs text-secondary">资产子类
                <select className={`${SELECT_CLASS} mt-1`} value={tradeForm.assetSubcategory} disabled={(categoryByCode.get(tradeForm.assetCategory)?.subcategories || []).length === 0} onChange={(e) => updateTradeForm({ assetSubcategory: e.target.value as AssetSubcategory, ...(tradeForm.side === 'sell' ? { symbol: '', name: '' } : {}) })}>
                  {(categoryByCode.get(tradeForm.assetCategory)?.subcategories || []).length > 0 ? categoryByCode.get(tradeForm.assetCategory)!.subcategories.map((option) => <option key={option.code || 'empty'} value={option.code}>{option.name}</option>) : <option value="">--</option>}
                </select>
              </label>
              ...
              <label className="text-xs text-secondary">方向
                <select className={`${SELECT_CLASS} mt-1`} value={tradeForm.side} onChange={(e) => updateTradeForm({ side: e.target.value as 'buy' | 'sell', symbol: '', name: '' })}>
                  <option value="buy">买入</option>
                  <option value="sell">卖出</option>
                </select>
              </label>
              {tradeForm.side === 'sell' ? (
                <label className="text-xs text-secondary">卖出标的
                  <select className={`${SELECT_CLASS} mt-1`} required value={tradeForm.symbol} onChange={(e) => handleSellPositionChange(e.target.value)} disabled={!selectedAccount || selectedSellPositions.length === 0}>
                    <option value="">{selectedSellPositions.length > 0 ? '选择已有持仓' : '当前条件无可卖持仓'}</option>
                    {selectedSellPositions.map((item) => <option key={`${item.accountId}-${item.market}-${item.symbol}`} value={item.symbol}>{item.symbol} {item.name || ''} / 可卖 {formatNumber(item.quantity)}</option>)}
                  </select>
                </label>
              ) : (
              ...
              <label className="text-xs text-secondary">标的名称
                <input className={`${INPUT_CLASS} mt-1`} placeholder="资产名称" value={tradeForm.name} onChange={(e) => updateTradeForm({ name: e.target.value })} readOnly={tradeForm.side === 'sell'} />
              </label>
              ...
              <label className="text-xs text-secondary">开放日
                <input className={`${INPUT_CLASS} mt-1`} type="date" value={tradeForm.availableDate} onChange={(e) => updateTradeForm({ availableDate: e.target.value })} />
              </label>
            ...
            <div className="mt-auto flex items-center justify-between gap-2 rounded-lg border border-border/40 bg-surface/40 px-3 py-2 text-xs text-secondary">
              ...
              <Button type="submit" size="sm" variant="primary" isLoading={submitting === 'trade'} disabled={!selectedAccount || (tradeForm.side === 'sell' && !tradeForm.symbol)} loadingText="保存中...">保存交易</Button>
            </div>
          </form>
        </Card>
        ) : null}

        {activeEventForm === 'cash' ? (
        <Card className="!rounded-xl" padding="sm">
          <form className="flex h-full flex-col gap-2" onSubmit={handleCashSubmit}>
            ...
            <div className="mt-auto rounded-lg border border-border/40 bg-surface/40 px-3 py-2 text-xs text-secondary">
              <div className="flex items-center justify-between gap-2">
                ...
                <Button type="submit" size="sm" variant="primary" isLoading={submitting === 'cash'} disabled={!selectedAccount} loadingText="保存中...">保存流水</Button>
              </div>
            </div>
          </form>
        </Card>
          ...
          <form className="flex h-full flex-col gap-2" onSubmit={handleCorporateSubmit}>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="text-xs text-secondary">标的
                <select className={`${SELECT_CLASS} mt-1`} required value={corpForm.symbol} onChange={(e) => updateCorpForm({ symbol: e.target.value })}>
                  ...
                  {selectedDividendPositions.map((item) => <option key={`${item.accountId}-${item.market}-${item.symbol}`} value={item.symbol}>{item.symbol} {item.name || ''}</option>)}
                </select>
              </label>
            ...
            <div className="mt-auto rounded-lg border border-border/40 bg-surface/40 px-3 py-2 text-xs text-secondary">
              <div className="flex items-center justify-between gap-2">
                ...
                <Button type="submit" size="sm" variant="primary" isLoading={submitting === 'corporate'} disabled={!selectedAccount || !selectedCorporatePosition} loadingText="保存中...">保存分红</Button>
              </div>
            </div>
          </form>
        </Card>
        ...
        {activeEventForm === 'split' ? (
        <Card className="!rounded-xl" padding="sm">
          <form className="flex h-full flex-col gap-2" onSubmit={handleSplitSubmit}>
            <div>
              <p className="text-xs uppercase tracking-[0.22em] text-secondary">Stock Split</p>
              <h2 className="mt-1 text-base font-semibold text-foreground">送股除权</h2>
              <p className="mt-1 text-sm text-secondary">只针对股票类持仓，保存后按比例调整数量、成本和当前价。</p>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <label className="text-xs text-secondary">账户
                <select className={`${SELECT_CLASS} mt-1`} value={selectedAccountId} onChange={(e) => setSelectedAccountId(e.target.value ? Number(e.target.value) : '')} disabled={loading}>
              ...
              <div className="text-xs text-secondary">市场/币种
                <div className="mt-1 flex h-8 items-center rounded-lg border border-border/40 bg-background/40 px-2.5 text-xs text-foreground">
                  {selectedSplitPosition ? `${selectedSplitPosition.market.toUpperCase()} / ${selectedSplitPosition.currency}` : selectedAccount ? `${selectedAccount.market.toUpperCase()} / ${selectedAccount.baseCurrency}` : '--'}
                </div>
              </div>
              <label className="text-xs text-secondary">标的
                <select className={`${SELECT_CLASS} mt-1`} required value={splitForm.symbol} onChange={(e) => updateSplitForm({ symbol: e.target.value })} disabled={!selectedAccount || selectedStockPositions.length === 0}>
                  <option value="">{selectedStockPositions.length > 0 ? '选择已有股票持仓' : '当前账户无股票持仓'}</option>
                  {selectedStockPositions.map((item) => <option key={`${item.accountId}-${item.market}-${item.symbol}`} value={item.symbol}>{item.symbol} {item.name || ''} / 持仓 {formatNumber(item.quantity)}</option>)}
                </select>
              </label>
              <label className="text-xs text-secondary">事件日期
                <input className={`${INPUT_CLASS} mt-1`} required type="date" value={splitForm.effectiveDate} onChange={(e) => updateSplitForm({ effectiveDate: e.target.value })} />
              </label>
              <div className="text-xs text-secondary">方向
                <div className="mt-1 flex h-8 items-center rounded-lg border border-border/40 bg-background/40 px-2.5 text-xs text-foreground">除权</div>
              </div>
              <label className="text-xs text-secondary">3股/10股
                <div className="mt-1 grid grid-cols-[1fr_auto] items-center gap-2">
                  <input className={INPUT_CLASS} required type="number" min="0" step="0.0001" value={splitForm.splitRatio} onChange={(e) => updateSplitForm({ splitRatio: e.target.value })} />
                  <span className="text-xs text-secondary">股 / 10股</span>
                </div>
              </label>
              <div className="text-xs text-secondary">当前持仓数量
                <div className="mt-1 flex h-8 items-center rounded-lg border border-border/40 bg-background/40 px-2.5 text-xs text-foreground">
                  {selectedSplitPosition ? formatNumber(selectedSplitPosition.quantity) : '--'}
                </div>
              </div>
              <div className="text-xs text-secondary">除权后数量
                <div className="mt-1 flex h-8 items-center rounded-lg border border-border/40 bg-background/40 px-2.5 text-xs text-foreground">
                  {selectedSplitPosition && stockSplitRatio > 0 ? formatNumber(selectedSplitPosition.quantity * (10 + stockSplitRatio) / 10) : '--'}
                </div>
              </div>
            </div>
            <label className="text-xs text-secondary">备注
              <input className={`${INPUT_CLASS} mt-1`} placeholder="可选" value={splitForm.note} onChange={(e) => updateSplitForm({ note: e.target.value })} />
            </label>
            <div className="mt-auto rounded-lg border border-border/40 bg-surface/40 px-3 py-2 text-xs text-secondary">
              <div className="flex items-center justify-between gap-2">
                <span>除权比例：每10股送 {formatNumber(stockSplitRatio)} 股</span>
                <Button type="submit" size="sm" variant="primary" isLoading={submitting === 'split'} disabled={!selectedAccount || !selectedSplitPosition || stockSplitRatio <= 0} loadingText="保存中...">保存除权</Button>
              </div>
            </div>
          </form>
        </Card>
      ...
      <ConfirmDialog
        isOpen={fundResetConfirmOpen}
        title="净值重置确认"
        message={fundResetConfirmMsg}
        confirmText="确认重置"
        cancelText="取消"
        onConfirm={handleFundResetConfirm}
        onCancel={() => { setFundResetConfirmOpen(false); }}
      />
    </AppPage>
  );
};
