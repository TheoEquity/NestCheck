...
- [新功能] 新增 `asset_risk_definitions` 表存储 R1-R5 风险等级定义（名称、预期回报率、波动率、最大回撤、权益权重），权益比率计算从数据库动态读取权重配置。
- [新功能] 新增资产配置页面 (`/assets/allocation`)，支持查看和编辑 R1-R5 风险等级定义，包含预期回报率、波动率、最大回撤和权益权重。
- [改进] 资产配置测算新增后端 SLSQP 求解接口，按 R1-R5 数据库定义计算目标配置，并在前端保留 1% 枚举兜底。
- [新功能] 资产配置新增配置计划表与计划管理卡片，支持保存 R1-R5 目标比例并选择单一生效版本。
...
- [改进] Web 收敛为轻量个人资产管理入口，关注标的聚焦红绿灯，新增资产诊断页，并将问股入口改为统一资产问答。
- [新功能] 关注标的页左栏新增行业 ETF 温度，支持固定行业的代表 ETF 配置，并展示日涨跌、近一月表现与相对沪深300强弱。
- [修复] 行业 ETF 温度的近一月涨跌幅改为使用最近 20 个交易日涨跌幅复合计算，避免 ETF 折算导致收盘价断层放大月度表现。
- [新功能] 关注标的新增 watchlist 专属 5 灯硬规则红绿灯，独立采集指标快照并在市场数据刷新任务中更新估值、利润质量、偿债、兑现力和周线节奏结果。
- [改进] 新增股票关注标的保存后立即初始化红绿灯快照，前端刷新按钮改为只读取已保存结果，并在技术灯历史不足时自动补齐日线数据。
- [修复] 关注标的股票卡片显示带市场后缀的代码，并在缺少实时行情缓存时回退显示最新日线收盘价与涨跌幅。
- [改进] Watchlist 红绿灯接入 Tushare 股票行业属性映射，并为金融股优化利润质量、偿债韧性与 ROE 取值口径。
- [改进] Watchlist 周线节奏灯改为 5 周均线与 10 周均线交叉判断，提升短中期节奏敏感度。
...
- [文档] README 新增 Windows 个人电脑两步部署说明，并提供 Docker 一键部署 PowerShell 脚本。
...
- [修复] 收紧 Web/API 安全默认值：默认启用管理员认证，配置读取脱敏，高危配置和 Agent 写接口强制管理员会话，并加固 SSRF、上传大小和前端渲染边界。
...
- [改进] AI 问答基金模式新增“同类基金有哪些”快捷问答，并用同大类、近3月/近1年/近3年五分位排行画像筛选相似基金。
- [改进] AI 问答将“指数”入口改为“市场”入口，A 股市场问答无需输入代码，并新增市场轻量问答与“大盘深度分析”按钮。
- [改进] 资产模块统一价格精度：内部组合单位净值和开放式基金净值保留 4 位，ETF/LOF 保留 3 位，股票价格、金额与份额按既定口径展示和写入。
- [修复] 资产事件净值重置、账户增删和现金流入流出统一改用资产管理页持仓台账总资产口径，确保份额计算纳入外币汇率折算。
- [新功能] 资产交易事件新增开放日字段，并新增开放日跟踪页按交易事件展示可卖日期资产，支持取消关注已过期项目。
- [改进] 资产管理页将原实时重估入口替换为开放日跟踪入口，并移除开放日跟踪的侧边栏独立菜单。
- [改进] 资产管理页统一将持仓未实现收益展示为“持仓收益”，卡片值与资产台账明细合计保持同一字段口径。
- [改进] 市场数据刷新后台定时任务新增每日 08:30 补拉，并将后台自动执行纳入定时任务历史和成功率统计。
- [改进] 市场数据刷新在更新价格、基金净值和汇率缓存后自动重算内部组合单位净值，保持资产份额不变并写入最新净值记录。
- [改进] 开放日跟踪页统一使用“持仓收益”文案，并新增按账户、资产大类、货币和开放日起止时间筛选。
- [改进] 资产事件交易表单的资产大类下拉改为完整读取后端资产大类接口返回值，不再排除现金类资产。
- [改进] 新增 `asset_category_definitions` 资产大类/细类定义表，资产初始化和资产事件表单统一从后端定义接口读取大类、细类和默认风险分类。
- [改进] 关注标的新增表单的资产大类和细类改为读取资产类别定义表，并限制新增关注仅可选择股票或基金。
- [改进] 关注标的列表拆分为股票和基金页签，默认展示股票，避免两类红绿灯标准混在同一列表。
- [修复] 关注标的基金卡片净值按 4 位小数展示，股票价格继续按 2 位小数展示。
- [修复] 资产事件页交易、资金流水和现金分红表单改为独立保存状态，资金流水保存后同步刷新持仓和内部基金状态。
- [改进] 资产事件交易表单在卖出方向下按账户、资产大类和子类下拉选择已有持仓，包含现金类资产条目；买入方向继续支持手工录入新资产。
- [新功能] 资产事件页新增送股除权事件，支持股票持仓按每 10 股送 N 股调整数量、成本价和当前价，并纳入公司行为台账。
- [改进] 资产大类定义表新增现金细类“活期”和“定期”，交易事件选择现金大类时可继续选择现金细类。
- [改进] 资产大类定义表新增股票细类“个股”“指数”“ETF”，用于归类场内可实时交易资产；基金大类继续用于场外基金细分。
- [改进] 资产事件页改为事件操作页签展开表单，默认展示交易台账，减少四类事件表单同时占屏。
