...
class AnalysisResult:
    """简化的分析结果数据结构（仅用于 Agent 模式）"""

    code: str
    name: str
    sentiment_score: int = 50
    trend_prediction: str = "未知"
...
class StockAnalysisPipeline:
    ...
    def __init__(
        ...
        # 打印实时行情/筹码配置状态
        logger.info(f"实时行情优先级: {self.config.realtime_source_priority}")
    ...
    def _generate_aggregate_report(
        ...
        """Generate aggregate report string from analysis results."""
        report_lines = []
        for r in results:
            report_lines.append(f"## {r.code} - {r.stock_name}")
            report_lines.append(f"评分: {r.sentiment_score}")
            report_lines.append(f"操作建议: {r.operation_advice}")
            report_lines.append("")
        return "\n".join(report_lines)
